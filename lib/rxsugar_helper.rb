module RXSugarHelper
  ...
  def RXSugarHelper.rxsugar_from_grammar(grammar_file = DEFAULT_GRAMMAR)
    xsg = File.readlines(grammar_file).to_s
    RXSugar.new(xsg)
  end

  def RXSugarHelper.xml_file_to_non_xml(xml_file = STDIN, grammar_file = DEFAULT_GRAMMAR, output = STDOUT)
  ...
  def RXSugarHelper.non_xml_file_to_xml(non_xml_file = STDIN, grammar_file = DEFAULT_GRAMMAR, output = STDOUT)
    rxsugar = rxsugar_from_grammar(grammar_file)
